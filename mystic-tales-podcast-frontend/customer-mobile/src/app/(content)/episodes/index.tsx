    ...
    if (episodeData.episodes.length === 0 || !episodeData.title) {
        return (
            <View style={styles.centerContainer}>
                <Text style={{ color: '#aee339', fontSize: 20, fontWeight: 'bold' }}>No Episode Available Now</Text>
        ...
        >
            <View className="w-full flex flex-row items-center py-5 ">
                ...
                {episodeData.from === "ShowDetails" && (
                    <Pressable onPress={() => router.back()}>
                        <View className="flex flex-row items-center gap-2">
                            <MaterialIcons name="arrow-back-ios-new" size={18} color={"#AEE339"} />
                ...
                {episodeData.from === "Feed" && (
                    <View className="flex flex-row items-center justify-between w-full">
                       ...
                       {/* tạo khoảng cách cho title ở giữa:))) */}
                        <View className="flex flex-row items-center gap-2">
                            <MaterialIcons name="arrow-back-ios-new" size={18} color={"#000"} />
                            <Text className="text-[#000] font-medium text-xl">
                                Home
                            </Text>
                        </View>
                    </View>
                ...
                <>
                    ...
                    <View className="w-full flex flex-col items-start gap-5">
                        {episodeData?.episodes.map((item, index) => (
                            <Pressable
                                key={item.Id}
                                onPress={() => router.push(`/(content)/episodes/details/${item.Id}`)}
                                style={[style.episodeContainer, style.borderBottom]}
                            >
                                <View className="w-[70%] justify-between ">
                                    <Text style={style.dateText}>{formatDate(item.ReleaseDate)}</Text>
                                    <View className="w-full gap-2">
                                        <Text className="text-white text-[18px] font-bold" numberOfLines={2}>
                                            {item.Name}
                                        </Text>
                                        <HtmlText html={item.Description} numberOfLines={3} color="#9c9a9aff" />
                                    </View>
                            ...
                            >
                                ...
                                <View className="w-[70%] justify-between ">
                                    ...
                                    <View className="w-full gap-2">
                                        ...
                                        <HtmlText html={item.Description} numberOfLines={1} color="#9c9a9aff" />
                                    </View>
