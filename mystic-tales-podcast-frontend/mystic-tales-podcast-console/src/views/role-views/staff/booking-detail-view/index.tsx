...
const BookingDetailView: FC<BookingDetailViewProps> = () => {
    ...
    const fetchBookingDetail = async () => {
        ...
        (async () => {
            setLoading(true);
            try {
                const bookingDetail = await getBookingDetail(staffAxiosInstance, Number(Id));
                if (!alive) return;
    ...
    const handleCancelRequest = async (IsAccepted: boolean) => {
        if (depositRefundRate < 0 || depositRefundRate > 100) {
            toast.error("Please enter a valid refund rate between 0 and 100")
            return
        }
        if (IsAccepted === false) {
            const alert = await confirmAlert("Are you sure to reject this request?");
            if (!alert.isConfirmed) return;
        }
        let payload;
        try {
            setIsSubmitting(true);
            if (Booking.CurrentStatus?.Name === 'Customer Cancel Request' && IsAccepted === true) {
                payload = {
                    BookingCancelValidationInfo: {
                        CustomerBookingCancelDepositRefundRate: depositRefundRate,
                    }
                };
            } else if (Booking.CurrentStatus?.Name === 'Podcast Buddy Cancel Request' && IsAccepted === true) {
                payload = {
                    BookingCancelValidationInfo: {
                        PodcastBuddyBookingCancelDepositRefundRate: depositRefundRate
                    }
                };
            } else {
                payload = {
                    BookingCancelValidationInfo: {
                        CustomerBookingCancelDepositRefundRate: null,
                        PodcastBuddyBookingCancelDepositRefundRate: null
                    }
                };
            }
            const res = await cancelRequest(staffAxiosInstance, Number(Id), IsAccepted, payload);
                ...
                <div className="booking-detail__content">
                    ...
                    {(Booking?.BookingAutoCancelledReason || Booking?.BookingManualCancelledReason || Booking.CurrentStatus?.Name === 'Customer Cancel Request' || Booking.CurrentStatus?.Name === 'Podcast Buddy Cancel Request') && (
                        <div className="booking-detail__cancel-section">
                            ...
                            {(Booking.CurrentStatus?.Name === 'Customer Cancel Request' || Booking.CurrentStatus?.Name === 'Podcast Buddy Cancel Request') && (
                                <div className="flex gap-2">
                                    <CButton
                                        color="success"
                                        variant="outline"
                                        className="booking-detail__action-btn booking-detail__action-btn--reject me-2"
                                    ...
                                    <CButton
                                        color="success"
                                        variant="outline"
                                        className="booking-detail__action-btn booking-detail__action-btn--resolve me-2"
                ...
                <div className="resolve-popup-overlay" >
                    <div className="resolve-popup" onClick={(e) => e.stopPropagation()}>
                        <div className="resolve-popup__header">
                            <h4 className="resolve-popup__title">Resolve Cancel Request</h4>
                            <button
                        ...
                        <div className="resolve-popup__body">
                            <div className="resolve-popup__field">
                                <label className="resolve-popup__label">
                                    Deposit Refund Rate <span className="text-danger">*</span>
                                </label>
                                <input
                                    type="number"
                                    min="0"
                                    ...
                                    placeholder="Enter Deposit Refund Rate"
                                    value={depositRefundRate}
                                    onChange={(e) => setDepositRefundRate(Number(e.target.value))}
                                    onKeyPress={(e) => {
                                        if (e.key === 'Enter') {
