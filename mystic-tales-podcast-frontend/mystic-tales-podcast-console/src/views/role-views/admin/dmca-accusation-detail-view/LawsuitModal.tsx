...
import { createLawsuit } from '@/core/services/dmca/dmca.service'
...
const LawsuitModal: FC<LawsuitModalProps> = ({ onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            LawsuitProofAttachFiles: attachFiles
        }
        try {
            const response = await createLawsuit(adminAxiosInstance, Number(id) || context.dmcaAccusationId  , payload)
            const sagaId = response?.data?.SagaInstanceId
            if (!sagaId) {
                toast.error("Add Lawsuit failed, please try again.")
                return
            }
            ...
            toast.error('Failed to add Lawsuit')
        } finally {
            setIsSubmitting(false)
        }
    }
