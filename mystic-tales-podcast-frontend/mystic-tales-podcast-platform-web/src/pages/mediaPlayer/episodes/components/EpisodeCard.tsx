...
const getTimeAgo = (dateString: string): React.ReactNode => {
  ...
  if (diffInDays === 0)
    return <p className="text-sm text-[#d9d9d9] mb-1">Released: Today</p>;
  if (diffInDays === 1)
    return <p className="text-sm text-[#d9d9d9] mb-1">Released: 1 day ago</p>;
  if (diffInDays < 0)
    return (
      <div className="flex items-center gap-2 mb-1 ">
        <p className="text-sm text-[#D9D9D9]">Early Access</p>
        <div className="relative flex items-center">
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center text-mystic-green">
                <IoInformationCircleOutline className="ml-1 cursor-help" />
              </div>
            </TooltipTrigger>
            <TooltipContent>
              <p>Will be released on {dateString}</p>
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0">
              ...
              <h4 className="font-bold text-lg text-white mb-2 leading-tight">
                {`Episode: ${episode.EpisodeOrder}`} - {episode.Name}
              </h4>
              <div
