...
namespace Architecture_1.Infrastructure.Services.Consul
{
    public class ConsulRegistrationHostedService : IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    // Sử dụng BaseUrl từ HealthCheck config thay vì APP_BASE_URL
                    var baseUrl = !string.IsNullOrEmpty(_consulHealthCheckConfig.BaseUrl)
                        ? _consulHealthCheckConfig.BaseUrl
                        : _appConfig.APP_BASE_URL;

                    var healthCheckPath = $"{baseUrl.TrimEnd('/')}{_appConfig.HEALTH_CHECK_ENDPOINT}";

                    Console.WriteLine($"Health Check Enabled: {healthCheckPath}");
