...
{
    public class ChannelTemporal7dMaxQueryMetricUpdateJob : BackgroundService
    {
        private readonly ConsulDistributedLockService _lockService;
        private readonly IBackgroundJobsConfig _jobsConfig;
        private readonly ILogger<ChannelTemporal7dMaxQueryMetricUpdateJob> _logger;
        ...
        // Job configuration
        private BackgroundJob _jobConfig => _jobsConfig.ChannelTemporal7dMaxQueryMetricUpdateJob;
        private CronExpression? _cronExpression;

        public ChannelTemporal7dMaxQueryMetricUpdateJob(
            ConsulDistributedLockService lockService,
            IBackgroundJobsConfig jobsConfig,
            ILogger<ChannelTemporal7dMaxQueryMetricUpdateJob> logger,
        ...
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    ...
                    nameof(ChannelTemporal7dMaxQueryMetricUpdateJob),
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Background job is disabled: {JobName}", nameof(ChannelTemporal7dMaxQueryMetricUpdateJob));
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("Background job cancelled: {JobName}", nameof(ChannelTemporal7dMaxQueryMetricUpdateJob));
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Unexpected error in background job execution loop: {JobName}", nameof(ChannelTemporal7dMaxQueryMetricUpdateJob));
                    await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
                }
            }

            _logger.LogInformation("Background job stopped: {JobName}", nameof(ChannelTemporal7dMaxQueryMetricUpdateJob));
        }
        ...
        private async Task ExecuteJobWithLockAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var lockOptions = new ConsulLockOptions
                {
                    ...
                    {
                        ["JobName"] = nameof(ChannelTemporal7dMaxQueryMetricUpdateJob),
        ...
        private async Task ExecuteJobLogicAsync(string executionId, CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    await metricService.UpdateChannelTemporal7dMaxQueryMetric();
                }
