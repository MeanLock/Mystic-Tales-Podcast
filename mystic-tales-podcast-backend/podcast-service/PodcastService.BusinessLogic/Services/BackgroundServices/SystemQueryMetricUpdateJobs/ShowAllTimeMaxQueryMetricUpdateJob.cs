...
{
    public class ShowAllTimeMaxQueryMetricUpdateJob : BackgroundService
    {
        private readonly ConsulDistributedLockService _lockService;
        private readonly IBackgroundJobsConfig _jobsConfig;
        private readonly ILogger<ShowAllTimeMaxQueryMetricUpdateJob> _logger;
        ...
        // Job configuration
        private BackgroundJob _jobConfig => _jobsConfig.ShowAllTimeMaxQueryMetricUpdateJob;
        private CronExpression? _cronExpression;

        public ShowAllTimeMaxQueryMetricUpdateJob(
            ConsulDistributedLockService lockService,
            IBackgroundJobsConfig jobsConfig,
            ILogger<ShowAllTimeMaxQueryMetricUpdateJob> logger,
        ...
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                Console.WriteLine("ShowAllTimeMaxQueryMetricUpdateJob - StartAsync - Parsing cron expression: " + _jobConfig.CronExpression);
                ...
                _logger.LogInformation(
                    ...
                    nameof(ShowAllTimeMaxQueryMetricUpdateJob),
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Background job is disabled: {JobName}", nameof(ShowAllTimeMaxQueryMetricUpdateJob));
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("Background job cancelled: {JobName}", nameof(ShowAllTimeMaxQueryMetricUpdateJob));
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Unexpected error in background job execution loop: {JobName}", nameof(ShowAllTimeMaxQueryMetricUpdateJob));
                    await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
                }
            }

            _logger.LogInformation("Background job stopped: {JobName}", nameof(ShowAllTimeMaxQueryMetricUpdateJob));
        }
        ...
        private async Task ExecuteJobWithLockAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var lockOptions = new ConsulLockOptions
                {
                    ...
                    {
                        ["JobName"] = nameof(ShowAllTimeMaxQueryMetricUpdateJob),
        ...
        private async Task ExecuteJobLogicAsync(string executionId, CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    await metricService.UpdateShowAllTimeMaxQueryMetric();
                }
