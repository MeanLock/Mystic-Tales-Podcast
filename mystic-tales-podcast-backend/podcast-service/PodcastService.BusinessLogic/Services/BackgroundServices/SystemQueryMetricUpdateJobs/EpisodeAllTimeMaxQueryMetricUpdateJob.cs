...
{
    public class EpisodeAllTimeMaxQueryMetricUpdateJob : BackgroundService
    {
        private readonly ConsulDistributedLockService _lockService;
        private readonly IBackgroundJobsConfig _jobsConfig;
        private readonly ILogger<EpisodeAllTimeMaxQueryMetricUpdateJob> _logger;
        ...
        // Job configuration
        private BackgroundJob _jobConfig => _jobsConfig.EpisodeAllTimeMaxQueryMetricUpdateJob;
        private CronExpression? _cronExpression;

        public EpisodeAllTimeMaxQueryMetricUpdateJob(
            ConsulDistributedLockService lockService,
            IBackgroundJobsConfig jobsConfig,
            ILogger<EpisodeAllTimeMaxQueryMetricUpdateJob> logger,
        ...
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    ...
                    nameof(EpisodeAllTimeMaxQueryMetricUpdateJob),
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Background job is disabled: {JobName}", nameof(EpisodeAllTimeMaxQueryMetricUpdateJob));
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("Background job cancelled: {JobName}", nameof(EpisodeAllTimeMaxQueryMetricUpdateJob));
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Unexpected error in background job execution loop: {JobName}", nameof(EpisodeAllTimeMaxQueryMetricUpdateJob));
                    await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
                }
            }

            _logger.LogInformation("Background job stopped: {JobName}", nameof(EpisodeAllTimeMaxQueryMetricUpdateJob));
        }
        ...
        private async Task ExecuteJobWithLockAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var lockOptions = new ConsulLockOptions
                {
                    ...
                    {
                        ["JobName"] = nameof(EpisodeAllTimeMaxQueryMetricUpdateJob),
        ...
        private async Task ExecuteJobLogicAsync(string executionId, CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    // await metricService.UpdateChannelAllTimeMaxQueryMetric();
                    await metricService.UpdateEpisodeAllTimeMaxQueryMetricAsync();
                }
