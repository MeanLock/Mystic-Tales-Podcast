...
{
    public class EpisodeEditRequirementPendingTimeoutJob : BackgroundService
    {
        private readonly ConsulDistributedLockService _lockService;
        private readonly IBackgroundJobsConfig _jobsConfig;
        private readonly ILogger<EpisodeEditRequirementPendingTimeoutJob> _logger;
        ...
        // Job configuration
        private BackgroundJob _jobConfig => _jobsConfig.EpisodeEditRequirementPendingTimeoutJob;
        private CronExpression? _cronExpression;

        public EpisodeEditRequirementPendingTimeoutJob(
            ConsulDistributedLockService lockService,
            IBackgroundJobsConfig jobsConfig,
            ILogger<EpisodeEditRequirementPendingTimeoutJob> logger,
        ...
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    ...
                    nameof(EpisodeEditRequirementPendingTimeoutJob),
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Background job is disabled: {JobName}", nameof(EpisodeEditRequirementPendingTimeoutJob));
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("Background job cancelled: {JobName}", nameof(EpisodeEditRequirementPendingTimeoutJob));
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Unexpected error in background job execution loop: {JobName}", nameof(EpisodeEditRequirementPendingTimeoutJob));
                    await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
                }
            }

            _logger.LogInformation("Background job stopped: {JobName}", nameof(EpisodeEditRequirementPendingTimeoutJob));
        }
        ...
        private async Task ExecuteJobWithLockAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var lockOptions = new ConsulLockOptions
                {
                    ...
                    {
                        ["JobName"] = nameof(EpisodeEditRequirementPendingTimeoutJob),
        ...
        private async Task ExecuteJobLogicAsync(string executionId, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    var episodeService = scope.ServiceProvider.GetRequiredService<PodcastEpisodeService>();
                    await episodeService.RevertPendingEditRequiredEpisodesAsync();
                }
