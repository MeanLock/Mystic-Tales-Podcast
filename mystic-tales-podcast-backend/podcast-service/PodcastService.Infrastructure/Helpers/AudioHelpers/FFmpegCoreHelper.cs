...
namespace PodcastService.Infrastructure.Helpers.AudioHelpers
{
    public class FFmpegCoreHelper
    {
        ...
        public static async Task<double> GetAudioDurationSecondsFromStreamAsync(Stream audioStream)
        {
            try
            {
                // Tạo temp file từ stream
                ...
                // Ghi stream vào temp file
                await WriteStreamToTempfile(audioStream, tempPath);

                // Sử dụng hàm có sẵn để get duration
                var duration = await GetAudioDurationSecondsAsync(tempPath);
