...
//                 // Generate unique UID for this batch of segments
//                 var batchUid = Guid.NewGuid().ToString("N").Substring(0, 8);
//                 var segmentFileNamePattern = _hlsConfig.SegmentFileNamePattern.Replace("{uid}", batchUid);
//                 var segmentPattern = Path.Combine(hlsDir, segmentFileNamePattern);

//                 _logger.LogInformation($"Using segment pattern: {segmentFileNamePattern}");

//                 // ✅ Run FFmpeg WITHOUT encryption
//                 var success = await RunFfmpegHlsConversionNoEncryption(  // ← NEW METHOD
//                     audioFilePath,
//                     playlistPath,
//                     segmentPattern,
//                     segmentDuration,
...
//                 _logger.LogInformation("=== FFmpeg HLS Conversion Debug ===");
//                 _logger.LogInformation($"Input file: {audioFilePath}");
//                 _logger.LogInformation($"Output playlist: {playlistPath}");
//                 _logger.LogInformation($"Segment pattern: {segmentPattern}");
//                 _logger.LogInformation($"Segment duration: {segmentDuration}s");
//                 _logger.LogInformation($"Input format: {inputFormat.Format} ({inputFormat.Extension})");
...
//                         .FromFileInput(audioFilePath)
//                         .OutputToFile(playlistPath, overwrite: true, options => options
//                             // ⭐ STEP 1: Codec settings (MUST BE FIRST)
//                             .WithCustomArgument($"-c:a {codecArg}")
//                             .WithCustomArgument(bitrateArg != null ? $"-b:a {bitrateArg}" : "")
//                             .WithCustomArgument("-ar 44100")      // Sample rate
//                             .WithCustomArgument("-ac 2")          // Stereo

//                             // ⭐ STEP 2: Format specification (CRITICAL!)
//                             .WithCustomArgument("-f hls")         // Force HLS muxer

//                             // ⭐ STEP 3: HLS settings (AFTER codec and format)
//                             .WithCustomArgument($"-hls_time {segmentDuration}")
//                             .WithCustomArgument($"-hls_key_info_file \"{keyInfoPath}\"")
//                             .WithCustomArgument("-hls_playlist_type vod")
...
//                 try
//                 {
//                     var argumentString = ffmpegArgs.Arguments;
//                     _logger.LogInformation($"⭐ ACTUAL FFmpeg Command: ffmpeg {argumentString}");
//                 }
//                 catch (Exception logEx)
//                 {
//                     _logger.LogInformation($"Cannot log FFmpeg arguments: {logEx.Message}");
//                 }
...
//                     // ⭐ DEBUG: Check and log generated files
//                     await LogGeneratedFiles(Path.GetDirectoryName(playlistPath)!);

//                     // ⭐ DEBUG: Read and log playlist content
//                     await LogPlaylistContent(playlistPath);
//                     return true;
//                 }
//                 else
//                 {
...
//         /// <summary>
//         /// ⭐ FINAL FIX: Use Process directly instead of FFMpegCore
//         /// </summary>
//         private async Task<bool> RunFfmpegHlsConversionNoEncryption(
//     string audioFilePath,
//     string playlistPath,
//     string segmentPattern,
//     int segmentDuration,
...
//             try
//             {
//                 _logger.LogInformation("=== FFmpeg Direct Process (NO ENCRYPTION) ===");

//                 var (codecArg, bitrateArg, shouldCopyCodec) = HlsCodecStrategy.GetCodecStrategy(inputFormat);

//                 // ⭐ Build command with -vn to SKIP video streams
//                 var arguments = new List<string>
//         {
//             "-i", $"\"{audioFilePath}\"",
            
//             // ⭐⭐⭐ CRITICAL: Skip video streams (album art, etc.)
//             "-vn",                              // NO VIDEO!
...
//                 _logger.LogWarning($"⭐ Direct FFmpeg Command: ffmpeg {argumentString}");
...
//                 _logger.LogInformation("=== FFmpeg stderr (last 20 lines) ===");
//                 var lines = errorBuilder.ToString().Split('\n');
...
//                 {
//                     _logger.LogInformation("✅ FFmpeg conversion completed");
//                     await LogGeneratedFiles(Path.GetDirectoryName(playlistPath)!);
//                     await LogPlaylistContent(playlistPath);
//                     return true;
//                 }
...
//             catch (Exception ex)
//             {
//                 _logger.LogError(ex, "Error in FFmpeg conversion");
//                 return false;
//             }
//         }
//         #region LOGGING HLS PROCESSING DETAILS
//         /// <summary>
//         /// ⭐ NEW: Debug helper - Log generated files
//         /// </summary>
//         private async Task LogGeneratedFiles(string hlsDir)
//         {
//             try
//             {
//                 _logger.LogInformation("=== Generated Files ===");

//                 var allFiles = Directory.GetFiles(hlsDir, "*.*", SearchOption.TopDirectoryOnly);
//                 _logger.LogInformation($"Total files in HLS directory: {allFiles.Length}");
...
//                     _logger.LogInformation($"  - {fileName}: {fileInfo.Length} bytes ({extension})");

//                     // ⭐ For .ts segments, check duration with ffprobe
//                     if (extension == ".ts")
//                     {
//                         try
//                         {
//                             var segmentDuration = await GetSegmentDuration(file);
//                             _logger.LogInformation($"    → Actual segment duration: {segmentDuration:F6}s");
//                         }
//                         catch (Exception ex)
//                         {
//                             _logger.LogWarning($"    → Cannot get segment duration: {ex.Message}");
//                         }
//                     }
//                 }
//             }
//             catch (Exception ex)
//             {
//                 _logger.LogError(ex, "Error logging generated files");
//             }
//         }

//         /// <summary>
//         /// ⭐ NEW: Debug helper - Log playlist content
//         /// </summary>
//         private async Task LogPlaylistContent(string playlistPath)
//         {
//             try
//             {
//                 if (!File.Exists(playlistPath))
//                 {
//                     _logger.LogError($"❌ Playlist file not found: {playlistPath}");
//                     return;
//                 }

//                 var playlistContent = await File.ReadAllTextAsync(playlistPath);

//                 _logger.LogWarning("=== Playlist Content ===");
//                 _logger.LogWarning(playlistContent);
//                 _logger.LogWarning("=== End Playlist ===");

//                 // ⭐ Parse and analyze EXTINF values
//                 var lines = playlistContent.Split('\n');
//                 var extinfLines = lines.Where(l => l.StartsWith("#EXTINF:")).ToList();

//                 _logger.LogInformation($"Found {extinfLines.Count} EXTINF entries:");
//                 foreach (var line in extinfLines)
//                 {
//                     _logger.LogInformation($"  {line}");

//                     // Extract duration value
//                     var match = System.Text.RegularExpressions.Regex.Match(line, @"#EXTINF:([\d.]+)");
//                     if (match.Success && double.TryParse(match.Groups[1].Value, out var duration))
//                     {
//                         if (duration < 0.1)
//                         {
//                             _logger.LogError($"  ❌ DETECTED INVALID DURATION: {duration}");
//                         }
//                         else
//                         {
//                             _logger.LogInformation($"  ✅ Duration OK: {duration}s");
//                         }
//                     }
//                 }
//             }
//             catch (Exception ex)
//             {
//                 _logger.LogError(ex, "Error logging playlist content");
//             }
//         }

//         /// <summary>
//         /// ⭐ NEW: Get actual segment duration using ffprobe
//         /// </summary>
//         private async Task<double> GetSegmentDuration(string segmentPath)
//         {
//             try
//             {
//                 var ffOptions = new FFOptions
...
//                 var mediaInfo = await FFProbe.AnalyseAsync(segmentPath, ffOptions);
