...
//             var maxSimilarity = CombineSimilarityScores(similarities, fingerprint1, fingerprint2);
...
//             const int BIT_TOLERANCE = 6; // Allow n-bit differences per uint32, BIT_TOLERANCE càng thấp càng chặt (0=exact match, 1=very strict, 3=moderate, 5=loose)
...
//         /// <summary>
//         /// Intelligent similarity combination với logic rules
//         /// </summary>
//         private float CombineSimilarityScores(List<float> similarities, uint[] fp1, uint[] fp2)
//         {
//             float subseq = similarities[0];
//             float sliding = similarities[1];
//             float hamming = similarities[2];
//             float crossCorr = similarities[3];

//             Console.WriteLine($"Raw scores: Subseq={subseq:F1}%, Sliding={sliding:F1}%, Hamming={hamming:F1}%, CrossCorr={crossCorr:F1}%");

//             // RULE 1: Nếu SubsequenceMatch rất cao → Tin tưởng hoàn toàn
//             if (subseq >= 90f)
//             {
//                 Console.WriteLine($"✓ High confidence: SubsequenceMatch = {subseq:F1}%");
//                 return subseq;
//             }

//             // RULE 2: Nếu SubsequenceMatch trung bình-cao → Ưu tiên SubseqMatch + SlidingWindow
//             if (subseq >= 60f)
//             {
//                 float result = Math.Max(subseq, sliding);
//                 Console.WriteLine($"✓ Medium confidence: Max(Subseq, Sliding) = {result:F1}%");
//                 return result;
//             }

//             // RULE 3: Nếu SubsequenceMatch thấp → Cần validation
//             if (subseq < 30f)
//             {
//                 // Check cross-validation giữa các methods
//                 if (IsConsistentLowSimilarity(similarities))
//                 {
//                     float result = Math.Max(subseq, sliding); // Tin SlidingWindow hơn CrossCorr
//                     Console.WriteLine($"✓ Consistent low similarity: {result:F1}%");
//                     return result;
//                 }

//                 // Nếu CrossCorr cao bất thường → Có thể false positive
//                 if (crossCorr > 50f && subseq < 20f && sliding < 30f)
//                 {
//                     float penalty = 0.6f; // Giảm 40% điểm CrossCorr
//                     float adjustedCrossCorr = crossCorr * penalty;
//                     float result = Math.Max(Math.Max(subseq, sliding), adjustedCrossCorr);
//                     Console.WriteLine($"⚠️  CrossCorr false positive suspected: {crossCorr:F1}% → {adjustedCrossCorr:F1}%, final={result:F1}%");
//                     return result;
//                 }
//             }

//             // RULE 4: Default weighted combination
//             float weightedResult = CalculateWeightedAverage(similarities, fp1, fp2);
//             Console.WriteLine($"✓ Weighted combination: {weightedResult:F1}%");
//             return weightedResult;
//         }

//         /// <summary>
//         /// Kiểm tra tính nhất quán của low similarity
//         /// </summary>
//         private bool IsConsistentLowSimilarity(List<float> similarities)
//         {
//             // Nếu ít nhất 3/4 methods đều cho kết quả thấp → Consistent
//             int lowCount = similarities.Count(s => s < 40f);
//             return lowCount >= 3;
//         }

//         /// <summary>
//         /// Weighted average với adaptive weights
//         /// </summary>
//         private float CalculateWeightedAverage(List<float> similarities, uint[] fp1, uint[] fp2)
//         {
//             float subseq = similarities[0];
//             float sliding = similarities[1];
//             float hamming = similarities[2];
...
//             // Adaptive weights based on audio characteristics
//             var weights = GetAdaptiveWeights(fp1, fp2, similarities);

//             float weightedSum =
//                 subseq * weights.SubsequenceWeight +
//                 sliding * weights.SlidingWeight +
//                 hamming * weights.HammingWeight +
//                 crossCorr * weights.CrossCorrWeight;

//             float totalWeight = weights.SubsequenceWeight + weights.SlidingWeight +
//                                weights.HammingWeight + weights.CrossCorrWeight;

//             float result = weightedSum / totalWeight;

//             Console.WriteLine($"Weights: Subseq={weights.SubsequenceWeight:F1}, Sliding={weights.SlidingWeight:F1}, " +
//                              $"Hamming={weights.HammingWeight:F1}, CrossCorr={weights.CrossCorrWeight:F1}");

//             return result;
//         }

//         /// <summary>
//         /// Tính adaptive weights dựa trên audio characteristics
//         /// </summary>
//         private (float SubsequenceWeight, float SlidingWeight, float HammingWeight, float CrossCorrWeight)
//             GetAdaptiveWeights(uint[] fp1, uint[] fp2, List<float> similarities)
//         {
//             float lengthRatio = (float)Math.Max(fp1.Length, fp2.Length) / Math.Min(fp1.Length, fp2.Length);

//             // Base weights
//             float subseqWeight = 1.0f;
//             float slidingWeight = 1.0f;
//             float hammingWeight = 0.7f;  // Always lower trust in Hamming
//             float crossCorrWeight = 1.0f;

//             // ADJUSTMENT 1: Length-based weighting
//             if (lengthRatio > 2.0f) // Very different lengths
//             {
//                 subseqWeight = 1.2f;    // Favor SubsequenceMatch
//                 slidingWeight = 1.1f;   // Favor SlidingWindow
//                 hammingWeight = 0.5f;   // Distrust Hamming more
//                 crossCorrWeight = 0.8f; // Slight distrust CrossCorr
//             }
//             else if (Math.Abs(fp1.Length - fp2.Length) <= 10) // Very similar lengths
//             {
//                 hammingWeight = 1.0f;   // Trust Hamming more
//             }

//             // ADJUSTMENT 2: Score dispersion-based weighting  
//             float maxSim = similarities.Max();
//             float minSim = similarities.Min();
//             float dispersion = maxSim - minSim;

//             if (dispersion > 40f) // High dispersion → methods disagree
//             {
//                 subseqWeight *= 1.2f;   // Trust exact matching more
//                 crossCorrWeight *= 0.7f; // Distrust fuzzy matching
//             }

//             // ADJUSTMENT 3: Absolute score-based weighting
//             if (similarities[3] > 60f && similarities[0] < 30f) // CrossCorr high, Subseq low → suspicious
//             {
//                 crossCorrWeight *= 0.5f; // Heavy penalty on CrossCorr
//             }

//             return (subseqWeight, slidingWeight, hammingWeight, crossCorrWeight);
//         }
