...
{
    public class AccountNonPodcasterAccessHandler : AuthorizationHandler<AccountNonPodcasterAccessRequirement>
    {
        private readonly HttpServiceQueryClient _httpServiceQueryClient;
        private readonly RedisSharedCacheService _redisSharedCacheService;
        public AccountNonPodcasterAccessHandler(
        ...
        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, AccountNonPodcasterAccessRequirement requirement)
        {
            ...
            else if (account.HasVerifiedPodcasterProfile == true)
            {
                Console.WriteLine($"0000000000000000000000000000000000000000000000000000000Account has verified podcaster profile: {account.HasVerifiedPodcasterProfile}");
                context.Fail();
            }
            else
            {
