...
using TransactionService.BusinessLogic.DTOs.Auth;
...
using TransactionService.BusinessLogic.DTOs.ViewModels.Mail;
using TransactionService.Infrastructure.Services.Google.Email;
using TransactionService.Infrastructure.Configurations.Google.interfaces;
        ...
        public async Task RegisterCustomer(CustomerRegisterDTO customerRegisterDTO)
        {
            ...
            using (var transaction = await _appDbContext.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    {
                        await _fileIOHelper.CopyFileToFileAsync(FilePathHelper.CombinePaths(_filePathConfig.ACCOUNt_IMAGE_PATH, "unknown"), FilePathHelper.CombinePaths(folderPath, "main"));
                    }
            ...
            try
            {
                ...
                var result = await Task.WhenAll(customers.Select(async item =>
                {
                    return new AccountListItemDTO
                    {
                        ...
                        MainImageUrl = await _fileIOHelper.GeneratePresignedUrlAsync(FilePathHelper.CombinePaths(_filePathConfig.ACCOUNt_IMAGE_PATH, item.Id.ToString(), "main")),
        ...
        public async Task<AccountDetailDTO> GetAccountById(int accountId)
        {
            try
            {
                ...
                return new AccountDetailDTO
                {
                    ...
                    MainImageUrl = await _fileIOHelper.GeneratePresignedUrlAsync(FilePathHelper.CombinePaths(_filePathConfig.ACCOUNt_IMAGE_PATH, account.Id.ToString(), "main")),
        ...
        public async Task DeactivateAccount(int accountId, bool isDeactivate)
        {
            using (var transaction = await _appDbContext.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    // }else
                    // {
                    //     account.DeactivatedAt = this._dateHelper.GetNowByAppTimeZone();
                    // }
