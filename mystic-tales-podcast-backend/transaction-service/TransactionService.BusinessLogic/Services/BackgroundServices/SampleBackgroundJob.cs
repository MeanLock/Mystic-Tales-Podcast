...
using TransactionService.Common.Configurations.Consul;
using TransactionService.Common.Configurations.Consul.interfaces;
using TransactionService.Infrastructure.Models.Consul.DistributedLock;
using TransactionService.Infrastructure.Services.Consul.DistributedLock;
using Microsoft.Extensions.DependencyInjection;

namespace TransactionService.BusinessLogic.Services.BackgroundServices
{
    public class SampleBackgroundJob : BackgroundService
    {
        ...
        // Job configuration
        private BackgroundJob _jobConfig => _jobsConfig.SampleBackgroundJob;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Background job is disabled: {JobName}", nameof(SampleBackgroundJob));
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("Background job cancelled: {JobName}", nameof(SampleBackgroundJob));
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Unexpected error in background job execution loop: {JobName}", nameof(SampleBackgroundJob));
                    await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
                }
            }

            _logger.LogInformation("Background job stopped: {JobName}", nameof(SampleBackgroundJob));
        }
        ...
        private async Task ExecuteJobWithLockAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var lockOptions = new ConsulLockOptions
                {
                    ...
                    {
                        ["JobName"] = nameof(SampleBackgroundJob),
