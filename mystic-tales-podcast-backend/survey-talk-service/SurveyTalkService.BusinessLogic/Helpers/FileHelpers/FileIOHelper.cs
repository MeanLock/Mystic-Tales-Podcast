...
using SurveyTalkService.Infrastructure.Services.AWS.S3;

namespace SurveyTalkService.BusinessLogic.Helpers.FileHelpers
{
    public class FileIOHelper
    {
        ...
        public async Task<string> GeneratePresignedUrlAsync(string filePath)
        {
            if (_appConfig.FILE_STORAGE_SRC == "awsS3")
            {
                ...
                return await _awsS3BinaryFileService.GeneratePresignedUrlAsync(normalizedFilePath);
            }
        ...
        public async Task<byte[]?> GetFileBytesAsync(string filePath)
        {
            if (_appConfig.FILE_STORAGE_SRC == "awsS3")
            {
                ...
                // if (!FilePathHelper.HasExtension(normalizedFilePath))
                // {
                //     var fullFileKey = await _awsS3BinaryFileService.GetFullFileKeyAsync(normalizedFilePath);
                //     Console.WriteLine("Full file key: " + fullFileKey);
                //     if (!string.IsNullOrEmpty(fullFileKey))
                //     {
                //         return await _awsS3BinaryFileService.GetFileBytesAsync(fullFileKey);
                //     }
                // }
                // else
                // {
